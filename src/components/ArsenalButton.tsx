...
import { ARSENAL_COLORS } from '../constants/arsenalColors'; // Импортируем ARSENAL_COLORS для доступа к градиентам
    ...
    return (
      <TouchableOpacity onPress={handlePress} disabled={disabled || loading} activeOpacity={0.8}>
        <LinearGradient
          colors={
            variant === 'primary'
              ? [ARSENAL_COLORS.gradients.primary[0], ARSENAL_COLORS.gradients.primary[1]]
              : [ARSENAL_COLORS.gradients.secondary[0], ARSENAL_COLORS.gradients.secondary[1]]
          }
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
          ...
          {loading ? (
            ...
            <Text
              style={
                [...getTextStyle(), text, (disabled || loading) && styles.disabledText] as any
              }>

...
export const HomeScreen: React.FC = () => {
  ...
  const getGreeting = () => {
    ...
    switch (user.role) {
      ...
      case UserRole.SMM_MANAGER:
        return `Добро пожаловать, SMM-менеджер ${user.name}!`;
      default:
        return `Добро пожаловать, ${user.name}!`;
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
          ...
          <Card.Content>
            ...
            {user && (
              <Paragraph style={styles.debugInfo}>
                <Text style={styles.dbLabel}>Роль пользователя:</Text> {user.role}
              </Paragraph>
            )}
          </Card.Content>
        </Card>
      ...
      {(user?.role === UserRole.COACH || user?.role === UserRole.PARENT) && (
        <AnimatedCard animationType="slide" delay={800}>
          <Card style={styles.card}>
            <Card.Content>

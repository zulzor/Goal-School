...
export const NetworkProvider: React.FC<NetworkProviderProps> = ({ children }) => {
  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      // Если соединение восстановлено и мы на веб-платформе, проверяем если мы на странице fallback
      if (Platform.OS === 'web' && state.isConnected && state.isInternetReachable) {
        // Проверяем, если мы на странице fallback
  ...
  useEffect(() => {
    ...
    if (Platform.OS === 'web' && NetInfoWeb) {
      ...
      // NetInfo.addEventListener возвращает функцию отмены подписки

...
import { UserRole } from '../types'; // Импортируем UserRole
...
import SubscriptionManagementScreen from '../screens/SubscriptionManagementScreen'; // Добавляем импорт
import BranchTransferScreen from '../screens/BranchTransferScreen'; // Добавляем импорт
import TrainingManagementScreen from '../screens/TrainingManagementScreen'; // Добавляем импорт

// Тестовые пользователи для веб-версии
const TEST_USERS = [
  { id: '1', name: 'Иванов Иван', role: UserRole.CHILD, email: 'ivanov@example.com' },
  { id: '2', name: 'Петров Петр', role: UserRole.COACH, email: 'petrov@example.com' },
  { id: '3', name: 'Сидоров Сидор', role: UserRole.PARENT, email: 'sidorov@example.com' },
  { id: '4', name: 'Администратор', role: UserRole.MANAGER, email: 'admin@example.com' },
  { id: '5', name: 'Менеджер', role: UserRole.MANAGER, email: 'manager@example.com' }, // Добавляем менеджера
];
...
const MainTabs: React.FC<{ user: any }> = ({ user }) => {
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        tabBarIcon: ({ focused, color, size }) => {
          ...
          if (route.name === 'Home') {
            ...
            iconName = focused ? 'news' : 'newsOutline';
          } else if (route.name === 'SubscriptionManagement') { // Добавляем иконку для абонементов
            iconName = focused ? 'ticket' : 'ticketOutline';
          } else if (route.name === 'BranchTransfer') { // Добавляем иконку для переводов
            iconName = focused ? 'swapHorizontal' : 'swapHorizontalOutline';
          } else if (route.name === 'TrainingManagement') { // Добавляем иконку для управления тренировками
            iconName = focused ? 'calendar' : 'calendarOutline';
          }
      ...
      {(user?.role === UserRole.MANAGER || user?.role === UserRole.SMM_MANAGER) && (
        <>
          ...
          />
          <Tab.Screen
            name="SubscriptionManagement" // Добавляем экран управления абонементами
            component={SubscriptionManagementScreen}
            options={{ title: 'Абонементы' }}
          />
          <Tab.Screen
            name="BranchTransfer" // Добавляем экран управления переводами
            component={BranchTransferScreen}
            options={{ title: 'Переводы' }}
          />
          <Tab.Screen
            name="TrainingManagement" // Добавляем экран управления тренировками
